...
#define JACK_MAX_FRAMES                                                        \
  (4294967295U) /* This should be UINT32_MAX, but C++ has a problem with that. \
                 */
...
enum JackOptions {
  ...
  /**
   * pass a SessionID Token this allows the sessionmanager to identify the
   * client again.
   */
  JackSessionID = 0x20
};
 ...
 */
typedef void (*JackLatencyCallback)(jack_latency_callback_mode_t mode,
                                    void *arg);
 ...
 * @param arg pointer to a client supplied structure
 */
typedef void (*JackPortRenameCallback)(jack_port_id_t port,
                                       const char *old_name,
 ...
 * @param code a status word, formed by OR-ing together the relevant @ref
 JackStatus bits.
 * @param reason a string describing the shutdown reason (backend failure,
 server crash... etc...).
 * Note that this string will not be available anymore after the callback
 returns, so possibly copy it.
 * @param arg pointer to a client supplied structure
 */
typedef void (*JackInfoShutdownCallback)(jack_status_t code, const char *reason,
                                         void *arg);
